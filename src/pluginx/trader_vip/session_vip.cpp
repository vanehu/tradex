...
Session::Session( TraderVIP_P* trader_vip_p )
	...
	m_trader_vip_p = trader_vip_p; // 不检测是否空指针
}
...
std::string Session::OnTradeRequest( Request* request, HANDLE_SESSION api_session ) {
	...
	if( Fix_Run( api_session ) ) {
		...
		//m_trader_vip_p->LogPrint( basicx::syslog_level::c_debug, results, FILE_LOG_ONLY ); // 查询类返回的数据可能会非常多
	...
	if( 120001 == func_id || 120002 == func_id || 120003 == func_id || 120004 == func_id ) { // 模拟成交
		...
		if( 120001 == func_id ) {
			...
			//////////////////// 发送给交易客户端 //////////////////// 委托回报
			{
				...
				results_json["commit_ret"] = 6; // 全部成交
				results_json["commit_msg"] = basicx::StringToUTF8( "全部成交" );
				m_trader_vip_p->CommitResult( 1, request->m_identity, NW_MSG_CODE_JSON, Json::writeString( m_json_writer, results_json ) ); // 回报统一用 NW_MSG_CODE_JSON 编码
			}
			//////////////////// 发送给交易客户端 ////////////////////
...
bool Session::CallBackEvent( HANDLE_CONN api_connect, HANDLE_SESSION api_session, long subscibe, int32_t func_id ) {
	...
	if( results != "" ) {
		m_trader_vip_p->CommitResult( 1, request->m_identity, NW_MSG_CODE_JSON, results ); // 回报统一用 NW_MSG_CODE_JSON 编码 // Trade：1、Risks：2
	}
	...
	//	m_trader_vip_p->CommitResult( 1, it_se->second, NW_MSG_CODE_JSON, results ); // 回报统一用 NW_MSG_CODE_JSON 编码 // Trade：1、Risks：2
	//}
