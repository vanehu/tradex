...
TraderAPE the_app; // 必须

// 预设 TraderAPE_P 函数实现

TraderAPE_P::TraderAPE_P()
	: m_location( "" )
	, m_info_file_path( "" )
	, m_cfg_file_path( "" )
...
void TraderAPE_P::SetGlobalPath() {
...
bool TraderAPE_P::ReadConfig( std::string file_path ) {
	std::string log_info;
...
void TraderAPE_P::LogPrint( basicx::syslog_level log_level, std::string& log_info, int32_t log_show/* = 0*/ ) {
	if( 0 == log_show ) {
		m_syslog->LogPrint( log_level, m_log_cate, "LOG>: APE " + log_info ); // 控制台
		m_sysrtm->LogTrans( log_level, m_log_cate, log_info ); // 远程监控
	}
	m_syslog->LogWrite( log_level, m_log_cate, log_info );
}

bool TraderAPE_P::Initialize() {
	SetGlobalPath();
	ReadConfig( m_cfg_file_path );

	m_thread_service = boost::make_shared<boost::thread>( boost::bind( &TraderAPE_P::CreateService, this ) );
	m_thread_user_request = boost::make_shared<boost::thread>( boost::bind( &TraderAPE_P::HandleUserRequest, this ) );

	return true;
}

bool TraderAPE_P::InitializeExt() {
	return true;
}

bool TraderAPE_P::StartPlugin() {
	std::string log_info;
...
bool TraderAPE_P::IsPluginRun() {
	return m_plugin_running;
}

bool TraderAPE_P::StopPlugin() {
	std::string log_info;
...
bool TraderAPE_P::UninitializeExt() {
	return true;
}

bool TraderAPE_P::Uninitialize() {
...
bool TraderAPE_P::AssignTask( int32_t task_id, int32_t identity, int32_t code, std::string& data ) {
	try {
		...
		if( !write_in_progress && true == m_service_running ) {
			m_service->post( boost::bind( &TraderAPE_P::HandleTaskMsg, this ) );
		}
...
void TraderAPE_P::CreateService() {
	std::string log_info;
...
void TraderAPE_P::HandleTaskMsg() {
	std::string log_info;
...
// 自定义 TraderAPE_P 函数实现

bool TraderAPE_P::InitFixInfo() {
	std::string log_info;
...
void TraderAPE_P::StartNetServer() {
	...
	for( size_t i = 0; i < cfg_basic->m_vec_server_server.size(); i++ ) {
		if( "trade_ape" == cfg_basic->m_vec_server_server[i].m_type && 1 == cfg_basic->m_vec_server_server[i].m_work ) {
...
void TraderAPE_P::OnNetServerInfo( basicx::NetServerInfo& net_server_info_temp ) {
}

void TraderAPE_P::OnNetServerData( basicx::NetServerData& net_server_data_temp ) {
	try {
		if( "trade_ape" == net_server_data_temp.m_node_type ) { // 节点类型必须和配置文件一致
...
int32_t TraderAPE_P::GetSessionID() { // int32_t->2147483647：(0000001~2147482)*1000+(000~999)
...
void TraderAPE_P::HandleUserRequest() {
	std::string log_info;
...
std::string TraderAPE_P::OnUserLogin( Request* request ) {
	std::string log_info;
...
std::string TraderAPE_P::OnUserLogout( Request* request ) {
	std::string log_info;
...
void TraderAPE_P::CommitResult( int32_t task_id, int32_t identity, int32_t code, std::string& data ) {
...
std::string TraderAPE_P::OnErrorResult( int32_t func_id, int32_t ret_code, std::string ret_info, int32_t ret_task, int32_t encode ) {
...
TraderAPE::TraderAPE()
	...
	, m_trader_ape_p( nullptr ) {
	try {
		m_trader_ape_p = new TraderAPE_P();
	}
	catch( ... ) {
	}
}
...
TraderAPE::~TraderAPE() {
	if( m_trader_ape_p != nullptr ) {
		delete m_trader_ape_p;
		m_trader_ape_p = nullptr;
	}
}

bool TraderAPE::Initialize() {
	return m_trader_ape_p->Initialize();
}

bool TraderAPE::InitializeExt() {
	return m_trader_ape_p->InitializeExt();
}

bool TraderAPE::StartPlugin() {
	return m_trader_ape_p->StartPlugin();
}

bool TraderAPE::IsPluginRun() {
	return m_trader_ape_p->IsPluginRun();
}

bool TraderAPE::StopPlugin() {
	return m_trader_ape_p->StopPlugin();
}

bool TraderAPE::UninitializeExt() {
	return m_trader_ape_p->UninitializeExt();
}

bool TraderAPE::Uninitialize() {
	return m_trader_ape_p->Uninitialize();
}

bool TraderAPE::AssignTask( int32_t task_id, int32_t identity, int32_t code, std::string& data ) {
	return m_trader_ape_p->AssignTask( task_id, identity, code, data );
}
